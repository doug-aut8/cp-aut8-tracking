...
import { openBannerModal } from "@/components/BannerModal";


export type BannerActionType = "none" | "link" | "cupom" | "modal";
...
export const useBannerAction = () => {
  ...
  const runAction = useCallback(
    async (type: string | undefined, value: string | undefined, target?: string | undefined) => {
      ...
      if (t === "modal") {
        let payload: { imagem?: string; texto?: string } = {};
        try {
          payload = v ? JSON.parse(v) : {};
        } catch {
          payload = { texto: v };
        }
        if (!payload.imagem && !payload.texto) return;
        openBannerModal(payload);
        return;
      }
