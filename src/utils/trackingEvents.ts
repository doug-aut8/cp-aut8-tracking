...
import { peekUserId } from '@/integrations/supabase/session';
import { trackProductEvent, trackCartEvent, type CartEventItem } from '@/services/productEventService';
...
const getFirebaseUid = (): string | null => peekUserId();
...
/** Converte itens do carrinho no formato do array JSONB `items` dos eventos de carrinho. */
const toCartEventItems = (cartItems: CartItem[]): CartEventItem[] =>
  cartItems.map((item) => ({
    product_id: item.id,
...
export const trackInitiateCheckout = (cartItems: CartItem[], totalValue: number) => {
  try {
    ...
    // Persist begin_checkout to Supabase (UM único evento por carrinho)
    if (cartItems.length > 0) {
      trackCartEvent('begin_checkout', toCartEventItems(cartItems), totalValue);
    }
...
export const trackAbandonedCart = (cartItems: CartItem[], totalValue: number) => {
  try {
    if (cartItems.length > 0) {
      trackCartEvent('abandoned_cart', toCartEventItems(cartItems), totalValue);
    }
...
export const trackCheckoutFinalize = (cartItems: CartItem[], totalValue: number, durationMs: number) => {
  try {
    if (cartItems.length > 0) {
      trackCartEvent('checkout_finalize', toCartEventItems(cartItems), totalValue);
    }
...
export const trackPurchase = (params: {
  ...
  try {
    ...
    // Persist purchase como UM único evento por pedido (itens no array JSONB)
    ...
    if (billableItems.length > 0) {
      trackCartEvent('purchase', toCartEventItems(billableItems));
    }
