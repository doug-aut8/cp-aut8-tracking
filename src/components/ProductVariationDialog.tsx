...
interface ProductVariationDialogProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onAddToCart: (item: MenuItem & { quantity?: number; selectedSize?: PizzaSize }, selectedVariationGroups: SelectedVariationGroup[], selectedBorder?: PizzaBorder | null, itemObservation?: string) => void;
...
const ProductVariationDialog: React.FC<ProductVariationDialogProps> = ({
  ...
  // Refs dos containers de cada seção (bordas + grupos) para rolar até elas ao expandir
  const sectionRefs = useRef<Record<string, HTMLDivElement | null>>({});
  const scrollContainerRef = useRef<HTMLDivElement | null>(null);
  const scrollSpacerRef = useRef<HTMLDivElement | null>(null);
  // Guarda a seção que deve receber foco após a expansão ser renderizada
  ...
  }, [isOpen, item.id, initialObservation]);
  ...
  const handleAddToCart = () => {
    ...
    onAddToCart(effectiveItem, nonZeroGroups, selectedBorder, itemObservation.trim() || undefined);
    onClose();
  };
  ...
  useEffect(() => {
    const key = pendingScrollSectionRef.current;
    if (!key || !expandedGroups[key]) return;
    ...
    const frameId = requestAnimationFrame(() => {
      ...
      if (!container || !section || !spacer) return;

      // Remove o espaço da rolagem anterior antes de medir a posição atual.
      spacer.style.height = "0px";

      const containerRect = container.getBoundingClientRect();
      const sectionRect = section.getBoundingClientRect();
      const targetTop = container.scrollTop + sectionRect.top - containerRect.top;
      const missingSpace = Math.max(
        0,
        targetTop + container.clientHeight - container.scrollHeight
      );

      // Os últimos accordions também precisam de conteúdo abaixo deles para
      // que o navegador consiga alinhar seus cabeçalhos ao topo do modal.
  ...
  const OBSERVATION_KEY = "__observation";
  const toggleSection = (key: string, defaultExpanded: boolean) => {
    const willExpand = !(expandedGroups[key] ?? defaultExpanded);
    pendingScrollSectionRef.current = willExpand ? key : null;
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="product-fullscreen-dialog w-screen h-[100dvh] max-w-none rounded-none sm:w-[95vw] sm:max-w-md sm:h-auto sm:max-h-[90dvh] sm:rounded-2xl p-0 overflow-hidden flex flex-col bg-white [&>button]:hidden">
        <div className="flex flex-col flex-1 min-h-0">
          <div ref={scrollContainerRef} className="flex-1 min-h-0 overflow-y-auto" style={{ WebkitOverflowScrolling: 'touch' }}>
            ...
            <div className="px-5 sm:px-6 pt-5 pb-4">
              ...
              {item.tipo === "pizza" && item.permiteCombinacao && onOpenPizzaCombination && !isHalfPizza && (
                <div className="mt-6 flex items-center gap-6">
                  <button type="button" className="flex items-center gap-2 cursor-default">
                    ...
                    <span className="text-[12.6px] font-bold">Inteira</span>
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                  ...
                  >
                    ...
                    <span className="text-[12.6px] font-bold">Meio a Meio</span>
                  </button>
                </div>
              )}
              ...
              <div className="mt-4 space-y-2">
                {(() => {
                  ...
                  return sections.map((section) => {
                    if (section.type === "borders") {
                      ...
                      return (
                        <div key="borders-section" ref={(el) => { sectionRefs.current[BORDERS_KEY] = el; }} className="rounded-lg bg-rose-50/70 overflow-hidden">
                          <button
                            type="button"
                            onClick={() => toggleSection(BORDERS_KEY, false)}
                            className="w-full h-11 flex items-center justify-between px-3 py-2"
                          >
                            <span className="flex items-center gap-2">
                              <ChevronRight className={`h-4 w-4 transition-transform ${bordersExpanded ? "rotate-90" : ""}`} />
                              <span className="text-base font-bold">Bordas</span>
                            </span>
                    ...
                    const defaultExpanded = isCollapsible ? false : true;
                    const isExpanded = isCollapsible ? (expandedGroups[group.id] ?? defaultExpanded) : true;
                    ...
                    const groupHeader = (
                        ...
                        {isCollapsible && (
                          ...
                          <span className="text-base font-bold truncate">{group.name}</span>
                        {showHalfOption && (
                    ...
                    return (
                      <div key={group.id} ref={(el) => { sectionRefs.current[group.id] = el; }} className="rounded-lg bg-rose-50/70 overflow-hidden">
                        {isCollapsible ? (
                          <button
                            type="button"
                            onClick={() => toggleSection(group.id, defaultExpanded)}
                            className="w-full h-11 flex items-center justify-between gap-2 px-3 py-2"
                          >
                            {groupHeader}
                          ...
                          <div className="w-full h-11 flex items-center justify-between gap-2 px-3 py-2">
                            {groupHeader}
                            <span className={`flex-shrink-0 text-[11px] font-medium px-2 py-0.5 rounded-md ${
                              groupStatus.isValid ? 'bg-green-100 text-green-800' : 'bg-muted text-muted-foreground'
                            }`}>
                ...
                <div
                  ref={(el) => { sectionRefs.current[OBSERVATION_KEY] = el; }}
                  className="rounded-lg bg-rose-50/70 overflow-hidden"
                >
                  <button
                    type="button"
                    onClick={() => toggleSection(OBSERVATION_KEY, false)}
                    className="w-full h-11 flex items-center justify-between px-3 py-2"
                  >
                    <span className="flex items-center gap-2">
                      <ChevronRight className={`h-4 w-4 transition-transform ${(expandedGroups[OBSERVATION_KEY] ?? false) ? "rotate-90" : ""}`} />
                      <span className="text-base font-bold">Observações</span>
                    </span>
                    <span className="text-[11px] font-medium px-2 py-0.5 rounded-md bg-muted text-muted-foreground">
                      Opcional
                    </span>
                  </button>
                  {(expandedGroups[OBSERVATION_KEY] ?? false) && (
                    <div className="px-4 pb-4 animate-accordion-down">
                      <Textarea
              ...
              {/* Espaço calculado para permitir alinhar até a última seção ao topo */}
              <div ref={scrollSpacerRef} aria-hidden="true" className="h-0" />
            </div>
          </div>

          {/* Rodapé fixo: quantidade à esquerda, adicionar ao carrinho à direita */}
          <div className="px-5 sm:px-6 py-[7px] border-t flex-shrink-0 bg-white">
            <div className="flex items-center justify-between gap-3">
              {!(item.variationGroups && item.variationGroups.length > 0 && item.variationGroups.some(g => g?.hideMainQuantitySelector && (groupVariations[g.id]?.length ?? 0) > 0)) ? (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="h-7 w-7 flex items-center justify-center text-primary disabled:opacity-40 touch-action-manipulation"
                    onClick={() => setItemQuantity(q => Math.max(1, q - 1))}
                    disabled={itemQuantity <= 1}
                  >
                    <Minus className="h-4 w-4" />
                  </button>
                  <span className="h-8 min-w-[2.2rem] px-2 rounded-xl bg-primary text-primary-foreground flex items-center justify-center font-extrabold text-sm">
                    {itemQuantity}
                  </span>
                  <button
                    type="button"
                    aria-label="Aumentar quantidade"
                    className="h-7 w-7 flex items-center justify-center text-primary touch-action-manipulation"
              ...
              <Button 
                ...
                disabled={!isValid}
                className="bg-food-green hover:bg-opacity-90 text-white font-bold text-sm rounded-xl h-9 px-4 flex-shrink-0"
