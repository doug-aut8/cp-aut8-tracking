...
import { useLayoutSettings, saveLayoutSetting, saveLayoutSettingsBatch, COLOR_KEYS, darkKey } from '@/hooks/useLayoutSettings';
import { useImageUpload } from '@/hooks/useImageUpload';
import { getAllCategories } from '@/services/categoryService';
import { Category } from '@/types/menu';
import { supabase } from '@/integrations/supabase/client';
import { invalidateCategoryColorsCache } from '@/hooks/useCategoryColors';
...
const Layout = () => {
  ...
  const [corFundoSecaoPesquisa, setCorFundoSecaoPesquisa] = useState('#f9fafb');
  ...
  const [corFontePrecoProduto, setCorFontePrecoProduto] = useState('#ff6600');
  const [corFonteApartirDe, setCorFonteApartirDe] = useState('#6b7280');
  ...
  const [corBotaoAdicionar, setCorBotaoAdicionar] = useState('#4CAF50');
  ...
  const [darkCategoryColors, setDarkCategoryColors] = useState<Record<string, { bgColor: string; fontColor: string }>>({});
  ...
  useEffect(() => {
    if (!loading) {
      ...
      setCorFundoSecaoPesquisa(settings.cor_fundo_secao_pesquisa);
      ...
      setCorFontePrecoProduto((settings as any).cor_fonte_preco_produto || '#ff6600');
      setCorFonteApartirDe((settings as any).cor_fonte_apartir_de || '#6b7280');
      ...
      setCorBotaoAdicionar(settings.cor_botao_adicionar);
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        ...
        const darkMap: Record<string, { bgColor: string; fontColor: string }> = {};
        cats.forEach((c) => {
          map[c.id] = { bgColor: '#ffffff', fontColor: '#1f2937' };
          darkMap[c.id] = { bgColor: '#1f1f1f', fontColor: '#f5f5f5' };
        });
        data?.forEach((row) => {
          if (row.chave.startsWith('dark_cat_bg_')) {
            const catId = row.chave.replace('dark_cat_bg_', '');
            if (darkMap[catId]) darkMap[catId].bgColor = row.valor || '#1f1f1f';
          } else if (row.chave.startsWith('dark_cat_font_')) {
            const catId = row.chave.replace('dark_cat_font_', '');
            if (darkMap[catId]) darkMap[catId].fontColor = row.valor || '#f5f5f5';
  ...
  const aplicarPaletaEscuraSugerida = () => {
    setDarkColors((prev) => ({
      ...
      cor_fundo_secao_pesquisa: '#111111',
  ...
  const handleCategoryColorChange = (catId: string, field: 'bgColor' | 'fontColor', value: string) => {
    const setter = temaEdicao === 'escuro' ? setDarkCategoryColors : setCategoryColors;
    setter((prev) => ({
      ...prev,
  ...
  const handleSave = async () => {
    if (!hydrated) {
      toast({ title: 'Aguarde', description: 'As configurações ainda estão carregando.' });
      return;
    }
    setSaving(true);
    try {
      const layoutEntries: Record<string, string> = {
        ...
        cover_banner1_url: coverBanner1Url,
        cover_banner2_url: coverBanner2Url,
        cover_banner2_action_type: coverBanner2ActionType,
        cover_banner2_action_value: coverBanner2ActionValue,
        cover_banner2_action_target: coverBanner2ActionTarget,
        cover_background_tipo: coverBackgroundTipo,
        cor_background_cover: corBackgroundCover,
        cover_background_imagem_url: coverBackgroundImagemUrl,
        [darkKey('cover_background_tipo')]: darkCoverBackgroundTipo,
        [darkKey('cover_background_imagem_url')]: darkCoverBackgroundImagemUrl,
        cover_botao_cor_fundo: coverBotaoCorFundo || corPrimaria,
        cover_botao_cor_icone: coverBotaoCorIcone,
        modo_escuro: modoEscuro ? 'true' : 'false',
        cor_primaria: corPrimaria,
        cor_secundaria: corSecundaria,
        cor_fonte: corFonte,
        cor_fonte_categorias: corFonteCategorias,
        cor_barra_menu_categorias: corBarraMenuCategorias,
        cor_fundo_secao_pesquisa: corFundoSecaoPesquisa,
        cor_fonte_menu_categorias: corFonteMenuCategorias,
        cor_fundo_item_menu_categorias: corFundoItemMenuCategorias,
        cor_fonte_titulos: corFonteTitulos,
        cor_fonte_titulo_produto: corFonteTituloProduto,
        cor_fonte_descricao_produto: corFonteDescricaoProduto,
        cor_fonte_preco_produto: corFontePrecoProduto,
        cor_fonte_apartir_de: corFonteApartirDe,
        cor_fonte_secundaria: corFonteSecundaria,
        cor_background: corBackground,
        cor_barra_botoes: corBarraBotoes,
        cor_botoes: corBotoes,
        cor_fonte_botoes: corFonteBotoes,
        cor_botao_adicionar: corBotaoAdicionar,
        cor_background_header: corBackgroundHeader,
        cor_chat_cabecalho: corChatCabecalho,
        cor_chat_fonte_cabecalho: corChatFonteCabecalho,
        cor_chat_fonte_baloes: corChatFonteBaloes,
        cor_fonte_caixa_texto_chat: corFonteCaixaTextoChat,
        cor_destaque_categoria_ativa: corDestaqueCategoriaAtiva,
        cor_fonte_categoria_ativa: corFonteCategoriaAtiva,
        layout_colunas_mobile: layoutColunasMobile,
        cupom_aplicado_mensagem: cupomAplicadoMensagem,
      };

      // Versões dark de cada cor
      COLOR_KEYS.forEach((k) => {
        layoutEntries[darkKey(k as string)] = darkColors[k as string] || (settings as any)[k];
      });

      // Cores por categoria
      Object.entries(categoryColors).forEach(([catId, colors]) => {
        layoutEntries[`cat_bg_${catId}`] = colors.bgColor;
        layoutEntries[`cat_font_${catId}`] = colors.fontColor;
      });
      Object.entries(darkCategoryColors).forEach(([catId, colors]) => {
        layoutEntries[`dark_cat_bg_${catId}`] = colors.bgColor;
        layoutEntries[`dark_cat_font_${catId}`] = colors.fontColor;
      });

      // Uma única requisição HTTP com tudo
      await saveLayoutSettingsBatch(layoutEntries);
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      setCorBotaoAdicionar('#4CAF50');
      setCorBackgroundHeader(DEFAULT_COLORS.corBackgroundHeader);

      // Save all default colors to database
      await saveLayoutSettingsBatch({
        cor_primaria: DEFAULT_COLORS.corPrimaria,
        cor_secundaria: DEFAULT_COLORS.corSecundaria,
        cor_fonte: DEFAULT_COLORS.corFonte,
        cor_background: DEFAULT_COLORS.corBackground,
        cor_fonte_categorias: DEFAULT_COLORS.corFonteCategorias,
        cor_fonte_titulos: DEFAULT_COLORS.corFonteTitulos,
        cor_fonte_secundaria: DEFAULT_COLORS.corFonteSecundaria,
        cor_barra_botoes: DEFAULT_COLORS.corBarraBotoes,
        cor_botoes: DEFAULT_COLORS.corBotoes,
        cor_fonte_botoes: DEFAULT_COLORS.corFonteBotoes,
        cor_botao_adicionar: '#4CAF50',
        cor_background_header: DEFAULT_COLORS.corBackgroundHeader,
      });
  ...
  const colorGroups: { title: string; fields: ColorField[] }[] = [
    ...
    },
    {
      ...
      fields: [
        ...
        { key: 'cor_fundo_secao_pesquisa', label: 'Fundo da Seção de Pesquisa', value: corFundoSecaoPesquisa, set: setCorFundoSecaoPesquisa },
    ...
    },
    {
      ...
      fields: [
        ...
        { key: 'cor_fonte_preco_produto', label: 'Cor do Valor do Item', value: corFontePrecoProduto, set: setCorFontePrecoProduto },
        { key: 'cor_fonte_apartir_de', label: 'Cor da Frase "a partir de"', value: corFonteApartirDe, set: setCorFonteApartirDe },
    ...
    },
    {
      ...
      fields: [
        ...
        { key: 'cor_botao_adicionar', label: 'Cor do Botão "Adicionar"', value: corBotaoAdicionar, set: setCorBotaoAdicionar },
      ],
    },
    {
      title: 'Cabeçalho',
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-6 max-w-2xl space-y-6">
            ...
            <div>
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div className="mt-2 p-2 rounded" style={{ backgroundColor: pv('cor_fundo_secao_pesquisa', corFundoSecaoPesquisa) }}>
                    <div className="h-7 rounded border bg-white px-2 flex items-center text-xs text-gray-400">Buscar no cardápio...</div>
                  </div>
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    <span className="text-base font-bold" style={{ color: pv('cor_fonte_titulo_produto', corFonteTituloProduto) }}>Nome do Produto</span>
                    <span className="text-xs font-medium px-3 py-1.5 rounded-full text-white inline-flex items-center gap-1" style={{ backgroundColor: pv('cor_botao_adicionar', corBotaoAdicionar) }}>＋ Adicionar</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <p className="text-xs text-gray-500">
              As cores abaixo são independentes para os modos claro e escuro selecionados acima.
            </p>
            {loadingCategories ? (
              ...
              categories.map((cat) => {
                const cc = temaEdicao === 'escuro'
                  ? darkCategoryColors[cat.id] || { bgColor: '#1f1f1f', fontColor: '#f5f5f5' }
                  : categoryColors[cat.id] || { bgColor: '#ffffff', fontColor: '#1f2937' };
                return (
