...
import { Megaphone as PageIcon } from "lucide-react";
...
const Marketing = () => {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        const [{ data, error }, { data: secretRow }] = await Promise.all([
          supabase
            ...
            .maybeSingle(),
          (supabase as any)
            .from("tags_rastreamento_privado")
        ...
        if (data) {
          ...
          setConfig({
            ...
            meta_access_token: (secretRow as any)?.meta_access_token || "",
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const { meta_access_token, ...publicConfig } = config;
      ...
      const { error: secretError } = await (supabase as any)
        .from("tags_rastreamento_privado")
        .upsert(
          { tag_id: 1, meta_access_token: meta_access_token || null },
          { onConflict: "tag_id" }
        );
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      <AdminPageHeader title="Marketing & Rastreamento" icon={PageIcon} iconBg="bg-pink-100" iconColor="text-pink-600" />
