...
const Index = () => {
  ...
  const handleDeepLink = useCallback(() => {
    ...
    const parentItem = menuItems.find((i) => i.submenuItemIds?.includes(itemId));

    const open = (el: Element, handle: { triggerClick: (id?: string) => void }, subId?: string) => {
      deepLinkHandled.current = true;
      el.scrollIntoView({ behavior: "smooth", block: "center" });
      setTimeout(() => {
        handle.triggerClick(subId);
    ...
    const tryOpen = () => {
      ...
      if (parentItem) {
        const parentEl = document.querySelector(`[data-product-id="${parentItem.id}"]`);
        const parentHandle = itemRefs.current[parentItem.id];
        if (parentEl && parentHandle) {
          open(parentEl, parentHandle, itemId);
          return;
        }
      }
        ...
        <div className="fixed bottom-20 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] max-w-md">
          <div className="flex items-center gap-3 rounded-xl border-2 border-orange-400 bg-card shadow-2xl px-4 py-3">
            ...
            <button
              ...
              className="shrink-0 h-8 w-8 rounded-full bg-orange-400 hover:bg-orange-600 flex items-center justify-center"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
