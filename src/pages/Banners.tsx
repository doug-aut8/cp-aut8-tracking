...
import { useLayoutSettings, saveLayoutSettingsBatch } from '@/hooks/useLayoutSettings';
...
const parseModalValue = (value: string): { imagem: string; texto: string } => {
  try {
    const parsed = value ? JSON.parse(value) : {};
    if (parsed && typeof parsed === 'object') {
      return { imagem: parsed.imagem || '', texto: parsed.texto || '' };
    }
  } catch {
    // valor antigo em texto puro
  }
  return { imagem: '', texto: value || '' };
};
...
const BannerActionFields: React.FC<BannerActionFieldsProps> = ({
  ...
  const modal = parseModalValue(type === 'modal' ? value : '');

  const updateModal = (patch: Partial<{ imagem: string; texto: string }>) => {
    onValueChange(JSON.stringify({ ...modal, ...patch }));
  };
    ...
    <div className="mt-2 p-3 rounded border bg-muted/30 space-y-2">
      ...
      {type === 'modal' && (
        <div className="space-y-2">
          <Label className="text-xs">Imagem do modal (opcional, proporção 2:1)</Label>
          {modal.imagem && (
            <img
              src={modal.imagem}
              alt="Imagem do modal"
          ...
          <Input type="file" accept="image/*" onChange={handleModalUpload} disabled={isUploading} />
          <Input
            placeholder="Ou cole a URL da imagem"
            value={modal.imagem}
            onChange={(e) => updateModal({ imagem: e.target.value })}
          />
          <Label className="text-xs">Texto do modal</Label>
          <Textarea
            placeholder="Escreva o texto do modal. Pode usar quebras de linha."
            value={modal.texto}
            onChange={(e) => updateModal({ texto: e.target.value })}
            rows={5}
          />
        </div>
      )}
  ...
  const handleSave = async () => {
    ...
    try {
      await saveLayoutSettingsBatch({
        empresa_banner_url: bannerUrl,
        empresa_banner_mobile_url: bannerMobileUrl,
        empresa_banner_extra1_url: bannerExtra1Url,
        empresa_banner_extra2_url: bannerExtra2Url,
        banner_extra_quantidade: bannerExtraQuantidade,
        banner_extra_tamanho: bannerExtraTamanho,
        banner_principal_action_type: bannerPrincipalActionType,
        banner_principal_action_value: bannerPrincipalActionValue,
        banner_principal_action_target: bannerPrincipalActionTarget,
        banner_extra1_action_type: bannerExtra1ActionType,
        banner_extra1_action_value: bannerExtra1ActionValue,
        banner_extra1_action_target: bannerExtra1ActionTarget,
        banner_extra2_action_type: bannerExtra2ActionType,
        banner_extra2_action_value: bannerExtra2ActionValue,
        banner_extra2_action_target: bannerExtra2ActionTarget,
        usar_mesma_imagem_mobile: usarMesmaImagemMobile ? 'true' : 'false',
        cupom_aplicado_mensagem: cupomAplicadoMensagem,
        popup_ativo: popupAtivo ? 'true' : 'false',
        popup_imagem_url: popupImagemUrl,
        popup_texto_fechar: popupTextoFechar,
        popup_texto_aplicar: popupTextoAplicar,
        popup_action_type: popupActionType,
        popup_action_value: popupActionValue,
        popup_action_target: popupActionTarget,
      });
