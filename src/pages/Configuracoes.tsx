...
import { Cog as PageIcon } from "lucide-react";
...
import { ArrowLeft, Clock, RefreshCw, MessageCircle, Bell, ShieldCheck, Radio, Activity, Printer, CreditCard, Home, Volume2, Upload, Play } from "lucide-react";
...
const Configuracoes = () => {
  ...
  const [autoPrintOnNewOrder, setAutoPrintOnNewOrder] = useState(true);
  const [savingAutoPrintNewOrder, setSavingAutoPrintNewOrder] = useState(false);
  const [somNovoPedidoEnabled, setSomNovoPedidoEnabled] = useState(false);
  const [savingSomToggle, setSavingSomToggle] = useState(false);
  const [somNovoPedidoUrl, setSomNovoPedidoUrl] = useState("");
  const [savingSomUrl, setSavingSomUrl] = useState(false);
  const [uploadingSom, setUploadingSom] = useState(false);
  const somFileInputRef = React.useRef<HTMLInputElement>(null);
  ...
  const handleSomUpload = async (file: File) => {
    setUploadingSom(true);
    try {
      const lastDot = file.name.lastIndexOf(".");
      const ext = lastDot > 0 ? file.name.substring(lastDot + 1) : "mp3";
      const fileName = `sons/novo-pedido-${Date.now()}.${ext}`;
      const { data, error } = await supabase.storage
        .from("imagens-cardapio")
        .upload(fileName, file, { cacheControl: "3600", upsert: false });
      if (error) throw error;
      const { data: urlData } = supabase.storage.from("imagens-cardapio").getPublicUrl(data.path);
      setSomNovoPedidoUrl(urlData.publicUrl);
      await saveConfigValue("som_novo_pedido_url", urlData.publicUrl);
      toast({ title: "Sucesso!", description: "Som enviado e salvo." });
    } catch (error: any) {
      toast({ title: "Erro no upload", description: error.message, variant: "destructive" });
    } finally {
      setUploadingSom(false);
      if (somFileInputRef.current) somFileInputRef.current.value = "";
    }
  };
  ...
  const [paginaInicial, setPaginaInicial] = useState<"cover" | "cardapio">("cover");
  const [savingPaginaInicial, setSavingPaginaInicial] = useState(false);

  const handlePaginaInicialChange = async (value: "cover" | "cardapio") => {
    setSavingPaginaInicial(true);
    const prev = paginaInicial;
    setPaginaInicial(value);
    try {
      const { error } = await supabase
        .from("configuracoes")
        .upsert(
          { chave: "pagina_inicial", valor: value, updated_at: new Date().toISOString() },
          { onConflict: "chave" }
        );
      if (error) throw error;
      toast({
        ...
        description:
          value === "cardapio"
            ? "A página inicial agora é o Cardápio. A Cover foi desativada."
            : "A página inicial agora é a Cover.",
      });
    } catch (error: any) {
      setPaginaInicial(prev);
      toast({ title: "Erro ao salvar", description: error.message, variant: "destructive" });
    } finally {
      setSavingPaginaInicial(false);
    }
  };
    ...
    try {
      ...
      if (data) {
        data.forEach((row) => {
          ...
          if (row.chave === "auto_print_on_new_order") setAutoPrintOnNewOrder(row.valor !== "false");
          if (row.chave === "som_novo_pedido_enabled") setSomNovoPedidoEnabled(row.valor === "true");
          if (row.chave === "som_novo_pedido_url" && row.valor) setSomNovoPedidoUrl(row.valor);
          ...
          if (row.chave === "pagina_inicial") setPaginaInicial(row.valor === "cardapio" ? "cardapio" : "cover");
        });
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 pt-6">
        <AdminPageHeader title="Configurações do Sistema" icon={PageIcon} iconBg="bg-slate-100" iconColor="text-slate-600" />
      </div>
      ...
      <div className="container mx-auto px-4 py-8 max-w-2xl space-y-6">
          ...
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>
                  Escolha qual página o cliente vê ao acessar a loja.
                </CardDescription>
              </div>
            </div>
          </CardHeader>
            ...
            <div className="space-y-3">
              <label className="flex items-center gap-3 p-3 rounded-lg border cursor-pointer hover:bg-muted/50 transition-colors">
                <input
                  ...
                  name="pagina_inicial"
                  value="cover"
                  checked={paginaInicial === "cover"}
                  disabled={savingPaginaInicial}
                  onChange={() => handlePaginaInicialChange("cover")}
                  className="h-4 w-4 accent-primary"
                />
                <div>
                  <p className="font-medium">Cover</p>
                  <p className="text-sm text-muted-foreground">
                    Página de capa com banners e botões de navegação.
                  </p>
                </div>
              </label>
              <label className="flex items-center gap-3 p-3 rounded-lg border cursor-pointer hover:bg-muted/50 transition-colors">
                <input
                  ...
                  name="pagina_inicial"
                  value="cardapio"
                  checked={paginaInicial === "cardapio"}
                  disabled={savingPaginaInicial}
                  onChange={() => handlePaginaInicialChange("cardapio")}
                  className="h-4 w-4 accent-primary"
                />
                <div>
                  <p className="font-medium">Cardápio</p>
                  <p className="text-sm text-muted-foreground">
                    O cliente é direcionado direto ao cardápio. A Cover fica desativada.
                  </p>
                </div>
              </label>
            </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>Define quando o pedido será impresso automaticamente.</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between p-3 rounded-lg border bg-muted/30">
              <div className="space-y-0.5 pr-4">
                <Label htmlFor="auto_print_new_order_toggle" className="text-base">Imprimir automaticamente ao chegar o pedido</Label>
                <p className="text-sm text-muted-foreground">
                  Quando habilitado, todo novo pedido recebido na tela de pedidos é impresso automaticamente.
                </p>
              </div>
              <Switch
                id="auto_print_new_order_toggle"
                checked={autoPrintOnNewOrder}
                disabled={savingAutoPrintNewOrder}
                onCheckedChange={async (checked) => {
                  setSavingAutoPrintNewOrder(true);
                  const prev = autoPrintOnNewOrder;
                  setAutoPrintOnNewOrder(checked);
                  try {
                    const { error } = await supabase
                      .from("configuracoes")
                      .upsert(
                        { chave: "auto_print_on_new_order", valor: checked ? "true" : "false", updated_at: new Date().toISOString() },
                        { onConflict: "chave" }
                      );
                    if (error) throw error;
                    toast({ title: "Sucesso!", description: `Impressão ao chegar o pedido ${checked ? "habilitada" : "desabilitada"}.` });
                  } catch (error: any) {
                    setAutoPrintOnNewOrder(prev);
                    toast({ title: "Erro ao salvar", description: error.message, variant: "destructive" });
                  } finally {
                    setSavingAutoPrintNewOrder(false);
                  }
                }}
              />
            </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle>Som de Novo Pedido</CardTitle>
                <CardDescription>Toca um alerta sonoro na tela de pedidos quando chega um novo pedido.</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between p-3 rounded-lg border bg-muted/30">
              <div className="space-y-0.5 pr-4">
                <Label htmlFor="som_novo_pedido_toggle" className="text-base">Tocar Som ao Chegar Pedido</Label>
                <p className="text-sm text-muted-foreground">
                  Quando habilitado, o som abaixo é reproduzido a cada novo pedido recebido.
                </p>
              </div>
              <Switch
                id="som_novo_pedido_toggle"
                checked={somNovoPedidoEnabled}
                disabled={savingSomToggle}
                onCheckedChange={async (checked) => {
                  setSavingSomToggle(true);
                  const prev = somNovoPedidoEnabled;
                  setSomNovoPedidoEnabled(checked);
                  try {
                    await saveConfigValue("som_novo_pedido_enabled", checked ? "true" : "false");
                    toast({ title: "Sucesso!", description: `Som ao chegar pedido ${checked ? "habilitado" : "desabilitado"}.` });
                  } catch (error: any) {
                    setSomNovoPedidoEnabled(prev);
                    toast({ title: "Erro ao salvar", description: error.message, variant: "destructive" });
                  } finally {
                    setSavingSomToggle(false);
                  }
                }}
              />
            </div>
            ...
            <div className="space-y-2">
              <Label htmlFor="som_novo_pedido_url">URL do som</Label>
              <Input
                id="som_novo_pedido_url"
                value={somNovoPedidoUrl}
                onChange={(e) => setSomNovoPedidoUrl(e.target.value)}
                placeholder="https://.../alerta.mp3"
              />
              <div className="flex flex-wrap gap-2">
                <Button
                  onClick={async () => {
                    setSavingSomUrl(true);
                    try {
                      await saveConfigValue("som_novo_pedido_url", somNovoPedidoUrl.trim());
                      toast({ title: "Sucesso!", description: "URL do som salva." });
                    } catch (error: any) {
                      toast({ title: "Erro ao salvar", description: error.message, variant: "destructive" });
                    } finally {
                      setSavingSomUrl(false);
                    }
                  }}
                  disabled={savingSomUrl}
                >
                  {savingSomUrl ? "Salvando..." : "Salvar URL"}
                </Button>
                <Button
                  variant="outline"
                  onClick={() => somFileInputRef.current?.click()}
                  disabled={uploadingSom}
                >
                  <Upload className="h-4 w-4 mr-2" />
                  {uploadingSom ? "Enviando..." : "Upload do som"}
                </Button>
                <Button
                  variant="outline"
                  disabled={!somNovoPedidoUrl.trim()}
                  onClick={() => {
                    const audio = new Audio(somNovoPedidoUrl.trim());
                    audio.play().catch(() =>
                      toast({ title: "Não foi possível tocar", description: "Verifique a URL do som.", variant: "destructive" })
                    );
                  }}
                >
