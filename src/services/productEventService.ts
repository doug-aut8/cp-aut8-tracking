...
/** Cidades estimadas presentes nos eventos de navegação. */
export const getVisitCityOptions = async (): Promise<string[]> => {
  const { data, error } = await supabase.rpc('mkt_cidade_options' as any);
  if (error) {
