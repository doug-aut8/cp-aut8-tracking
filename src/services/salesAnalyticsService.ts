...
export type LocationField = "cidade" | "bairro";
export interface LocationFilter {
  field: LocationField;
  value: string;
}

const applyLocation = (query: any, location?: LocationFilter | null) =>
  location?.value ? query.eq(location.field, location.value) : query;

const locationKey = (location?: LocationFilter | null) =>
  location?.value ? `${location.field}:${location.value}` : "";

/** Lista os valores distintos de cidade ou bairro registrados nos pedidos. */
export const fetchLocationOptions = async (field: LocationField): Promise<string[]> => {
  const { data, error } = await cachedFetch(
    `orders|locations|${field}`,
    async () =>
      await supabase
        .from("pedidos_sabor_delivery" as any)
        .select(field)
...
export const fetchSalesSummary = async (
  ...
  const { data, error } = await cachedFetch(
    `orders|summary|${startIso}|${endIso}|${sourceFilter ?? ""}|${campaignFilter ?? ""}|${locationKey(location)}`,
    async () => {
      let query = supabase
        .from("pedidos_sabor_delivery" as any)
      ...
      query = applyLocation(query, location);
      return await query;
    },
  );
  if (error) throw new Error(error.message);
...
export const fetchDailySalesMetrics = async (
  ...
  const { data, error } = await cachedFetch(
    `orders|daily_sales|${startIso}|${endIso}|${sourceFilter ?? ""}|${campaignFilter ?? ""}|${locationKey(location)}`,
    async () => {
      let query = supabase
        .from("pedidos_sabor_delivery" as any)
...
export const fetchOrderCountDetail = async (
  ...
  const { data, error } = await cachedFetch(
    `orders|count_detail|${startIso}|${endIso}|${sourceFilter ?? ""}|${campaignFilter ?? ""}|${locationKey(location)}`,
    async () => {
      let query = supabase
        .from("pedidos_sabor_delivery" as any)
...
export const fetchSalesReport = async (
  ...
  const { data, error } = await cachedFetch(
    `orders|report|${startIso}|${endIso}|${sourceFilter ?? ""}|${campaignFilter ?? ""}|${locationKey(location)}`,
    async () => {
      let query = supabase
        .from("pedidos_sabor_delivery" as any)
...
export const fetchProductsReport = async (
  ...
  const { data, error } = await cachedFetch(
    `orders|products_report|${startIso}|${endIso}|${sourceFilter ?? ""}|${campaignFilter ?? ""}|${locationKey(location)}`,
    async () => {
      let query = supabase
        .from("pedidos_sabor_delivery" as any)
