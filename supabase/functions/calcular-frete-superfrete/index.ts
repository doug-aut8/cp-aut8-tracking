...
async function getSuperfreteToken(): Promise<string | null> {
  ...
  const { data, error } = await supabase
    ...
    .select("superfrete_token")
    .not("superfrete_token", "is", null)
    .limit(1)
    .maybeSingle();

  if (error) {
    console.error("Erro ao buscar token Superfrete:", error.message);
    return null;
  }

  return (data as any)?.superfrete_token ?? null;
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (!body.cep_origem || !body.cep_destino || !body.package) {
      return new Response(
        JSON.stringify({
          error: "Parâmetros obrigatórios: cep_origem, cep_destino, package",
        }),
        {
          status: 400,
    ...
    if (!token) {
      return new Response(
        JSON.stringify({
          error: "Token Superfrete não configurado. Configure em Logística.",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
